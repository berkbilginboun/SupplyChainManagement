...
# Function to solve profit optimization
def solve_profit_optimization(firm_name, demand, promo_month=None):
    ...
    model += lpSum((promo_price if t == promo_month else selling_price) * demand[t] -
...
# Checking the 9 required combinations
...
best_qh_strategy = None
max_qh_profit = float("-inf") #negative infinity

for qh_promo, unilock_promo in possible_combinations:
    ...
    if qh_profit > max_qh_profit:
        max_qh_profit = qh_profit
        best_qh_strategy = (qh_promo, unilock_promo)

# Print the best strategy for Q&H
print(
    f"Best decision for Q&H (Coordinated with Unilock): QH Promo in {best_qh_strategy[0]}, Unilock Promo in {best_qh_strategy[1]}")
print(f"Maximum Q&H Profit: {max_qh_profit}")
